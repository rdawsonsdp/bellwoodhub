...
| 1.5 | **Encryption in transit** — TLS required for all database connections and API traffic | CIS 3.10 · NIST SC-8 · CSF PR.DS-02 | Supabase enforces SSL; app pool sets `ssl` on every connection (web/lib/db.ts). Gap 5.1 (certificate verification) closed 2026-07-03 — CA pinned via `SUPABASE_CA_CERT` |
...
| 1.7 | **Credential custody + caged send** — mailbox OAuth refresh tokens live in Supabase Vault (007), referenced service-role-only under deny-all RLS; read scopes (Mail.Read / gmail.readonly / calendar.readonly) plus `gmail.send` (added 2026-07-03, RD's deliberate L1.7 decision). Transmission is triple-gated, fail-closed: human Approve → `SEND_ENABLED` master switch → `SAFE_SEND_ALLOWLIST` recipient allowlist (ships = operator's own address). Every attempt audited (`draft.sent` / `draft.send.caged` / `draft.send.failed`); no delete/write/account-management scope exists anywhere | NIST IA-5, AC-6 · least privilege with an auditable, bounded send path | web/lib/send-cage.ts; web/app/api/approvals/route.ts; scope strings in web/lib/auth.ts; migrations/012 (applied) |
...
|---|---|---|---|---|
| 5.1 | **CLOSED 2026-07-03** — DB client TLS did not verify the server certificate (was `rejectUnauthorized: false` in web/lib/db.ts) | Was: low (TLS still on; attacker needs network position) | Done: pool now verifies against the pinned Supabase CA (`SUPABASE_CA_CERT` env, set on the pilot) | Done — confirm on the first live connection |
...
| 5.3 | **CLOSED 2026-07-03** — OAuth refresh tokens sat in a locked DB table rather than a managed vault | Was: low (service-role-only, RLS, encrypted at rest) | Done: migration 007 moves tokens into **Supabase Vault** (`vault.create_secret`; `pipeline.connector_accounts` holds only the vault secret id) with a token-store module reading via `vault.decrypted_secrets` | Done |
| 5.4 | **PARTIAL 2026-07-03** — no centralized log drain / alerting yet (cron failures could go quiet) | Medium for operations, low for data | Shipped: secret-gated ops-watch health endpoint (cron/ingest freshness) + ops runbook. Remaining: RD points an external uptime monitor at it (~5-min dashboard task) | External monitor before Tuesday |
| 5.5 | **DRAFT DELIVERED 2026-07-03** — no formal records-retention schedule adopted for the hub | Legal/records, not technical | docs/RETENTION_POLICY.md drafted for counsel (Local Records Act / FOIA frame, copies-vs-record-system, no-auto-delete posture, proposed pilot default); **counsel adopts, we don't** (LGL-1) | Counsel review before Tuesday's mayor-account consent |
| 5.6 | **CLOSED 2026-07-03** — `app.audit_log` append-only was enforced by revokes + convention; the table owner could technically modify rows | Was: low; standard for single-operator pilots | Done: migration 009 adds a DB trigger guard that rejects UPDATE/DELETE for every role, including the table-owning service role | Done |
...
| 5.9 | **CLOSED 2026-07-03** (new control, logged for the record) — live mode was permissive: a missing `DATABASE_URL` silently fell back to demo fixtures, so fixture data could masquerade as live data | Data-integrity for the pilot; none for the demo | Done: **strict live mode** — the pilot env sets `DEMO_MODE=0`, under which fixtures can never serve; a missing/unreachable DB is a hard error, not a silent fallback. The public demo (Production, keyless `DEMO_MODE=1`) is unchanged | Done |
| 5.10 | **DISCLOSED 2026-07-03** (scope change, not a gap) — Google Calendar read scope added for the pilot rehearsal | None new — read-only posture unchanged | `calendar.readonly` added alongside `gmail.readonly` on the Google connector (RD's account rehearses Gmail + Calendar before Tuesday's production accounts); no write or send scope exists on any connector (see 1.7) | Done |

## 6. One-line answers for the two headline questions

- **"Does the database isolation meet municipal standards?"** Yes — instance-level isolation (dedicated project) + default-deny row security + revoked anonymous roles is *stronger* than the shared-database-with-policies pattern most municipal SaaS runs on, and it's verified by an automated advisor scan with zero findings. As of 2026-07-03 the DB client also verifies the pinned Supabase CA (5.1), OAuth tokens live in Supabase Vault (5.3), and the audit ledger is trigger-guarded append-only even against the table owner (5.6).
- **"Does the Vercel configuration meet standards?"** Yes for a pilot — environment separation, encrypted secrets, SSO-walled non-production, token-gated machine endpoints, strict live mode on the pilot (`DEMO_MODE=0`, 5.9), TLS everywhere, SOC 2/ISO 27001 vendors — with the §5 register naming exactly what's left (as of 2026-07-03, primarily: activate app login, point an external monitor at the ops-watch endpoint, and counsel adoption of the retention draft, docs/RETENTION_POLICY.md).
