...
**Last updated:** 2026-07-22 — **The mayor's app receives the full product: executive dashboard + design system + the intelligence loop.** Applied migrations **018 (cos_notes) + 019 (run diagnostics)** to the mayor Supabase (verified: table + RLS + column), then fast-forwarded `main` `7059869→a398e8f` (~30 commits) → bellwood-mayor.vercel.app. The day's run (all verified on bellwood-dev first): `FEAT-42` **executive dashboard** replaces the editorial desktop layout (Gemini reference; house tokens, not Tailwind — preflight would reset every screen): Top Issues from ALL desks with an index, scrollable body, and the CoS ranking on **urgency·relevance·risk** with per-item rationale shown AND recorded to the audit ledger (`brief.rank`); metric/chart/matrix/events/agents widgets, all real data, every item drilling to detail; icon-rail nav (old wide menu retired, bubble tooltips); greeting letterhead (full-color Bellwood art) ABOVE the Ask bar; sticky CoS card. `FEAT-43` **one design system app-wide**: white surfaces + Bellwood-navy borders via theme-aware `--c-card`/`--c-cardbd` (dark themes intact), serif titles swept to sans across 14 components; Emails, Email Actions, Agents pages deep-converted. `FEAT-44` **agents-at-work theater**: honest animation (record→desks→brief) while the brief composes, and a live Run panel where each desk flips to '✓ filed' from its real run timestamp. `FEAT-36b` **Your Voice card** (Agents page): upload the Claude-exported voice skill → kind='voice' → attached to every draft-ceiling desk (auto-respond step 1; skill upload verified end-to-end, smoke test cleaned). Also: NEXT_PUBLIC_LIVE=1 was missing locally AND on bellwood-dev (silently hid all live-only UI) — **verify it's set on bellwood-mayor's Vercel env** or skills/instructions/run/voice surfaces won't render there. bellwood-dev.vercel.app remains the dev twin (auth on, RD's data). Prior: **2026-07-21 (night) — **The dev app went to the cloud, and the Brief became a product.** `DEP-5` **bellwood-dev.vercel.app is live** — RD's developer deployment (own Supabase test DB, RD's Gmail), same codebase as the mayor's app, auth ON with fresh secret, all 12 env vars, crons always-on; Google OAuth redirect URI added by RD. The evening's feature run (all committed on `agent-focus-and-sync-fixes`, deployed to bellwood-dev, **NOT yet on the mayor's `main`**): `FEAT-36` **voice notes for walk-ins** shipped end-to-end — hold Ask and talk (note-vs-question classifier, confirm-first, Haiku cleanup: "um remember to cut the tree on forrest street" → "Tree removal — Forrest Street"), pencil NoteButton in the top bar, YOUR NOTES block with check-off, notes woven into the CoS narrative with stale nudges (migrations 018 applied local; mayor DB pending). `FEAT-38` **the Brief front page**: Chomsky blackletter nameplate (Tribune reference, vendored SIL-OFL woff2), clock-tracking edition (Morning/Midday/Evening), live date+time dateline, drop-cap lede, kicker/serif-headline/deck articles, "Agent Analysis & Updates" band. `FEAT-39` **desk stories forward to the Brief agent** which ranks the page by importance to the mayor (deterministic urgency score + CoS model re-rank); every article carries its **byline** (avatar + desk name); story headlines open the AGENT'S REPORT (email demoted to a source chip); Mail Triage rows open the email. `FEAT-40` **story mode** — "Swipe the Brief": full-screen right-to-left snap cards on mobile (~90% of use). `FEAT-41` **Show the work (eval/tracing)**: migration 019 + runner instrumentation records every run's model, timing, tokens, inputs, FULL prompt + raw response, and validation drops; a brain-icon button on every report renders the trace (verified live: sonnet-4-6 · 39.3s · 4917/2503 tokens · 12 focus matches · 0 dropped). UX: Agents folded to a peek row on the Hub + moved into the main menu; "Needs You"→**Email Actions** (auto-replies land there later); mobile header shows live date/time instead of the app name; note-panel z-index fix; topbar stacking fix. **OPEN:** push tonight's run to the mayor's `main` (apply 018+019 to his Supabase FIRST); `FEAT-37` calendar slot at the top of the Brief (blocked on Calendar-API enable); local dev has `AUTH_ENABLED=0` for LAN phone testing — restore before demoing sign-in locally; mayor Outlook admin consent. Prior: **The Hub became an intelligence product, live on the mayor's dashboard.** `FEAT-34` **"Needs to Know"** — the Chief of Staff briefing is the Hub's lead section on both surfaces: a model-voiced narrative in the tunable CoS persona + ranked top email issues (each opens the real email) + actions required + upcoming events, assembled live from triage + drafts + the calendar mirror (`lib/morning-live.ts`; `/api/morning-summary` live branch was a fixture TODO). Replaces the two separate "Needs you"/"Needs you now" sections. Tested against the real mailbox (T1–T5 pass, 5/5 links resolve; `docs/NEEDS_TO_KNOW_TEST_PLAN.md`). `FEAT-35` **agent reports read like a news brief** — each digest story carries a mini-headline + key facts + NEW/UPDATE tag (UPDATE = advances a story the agent's memory tracks); backward-compatible schema. `BUG-8` the graceful `/auth/error` page was itself behind the session gate on bellwood-mayor (its audience has no session) — exempted in middleware, verified 200 live. All pushed to `main` → **bellwood-mayor.vercel.app** (`7059869`). **IN PROGRESS:** `FEAT-36` voice notes for walk-ins ("remember to cut the tree on Forrest Street") — spoken capture → Whisper → open follow-ups folded into the briefing; migration `018_cos_notes.sql` written, API/UI/briefing integration next; two-way voice conversation is the follow-on. **OPEN:** `FEAT-37` **calendar section at the TOP of the Brief** (RD 2026-07-21) — reserve the slot above the articles; details land when the calendar application is connected (blocked by the same Calendar-API enable below); Google Calendar API disabled in the OAuth project (403 — one-click enable, then events populate); mayor's Outlook one-time admin consent; CoS persona for Mayor Harvey = Admin → Chief of Staff → Instructions box (no code needed); local dev has auth OFF for phone testing (flip back before demoing auth). Prior: **2026-07-20 (late) — Mobile-readiness pass + Ask consolidation, deployed to the mayor's app.** `BUG-6` **agent card opened off-screen on mobile** — root cause: `PullToRefresh` applied `transform:translateY(0)` at rest, making it the containing block for every `position:fixed` modal and trapping their z-index below the nav; transform is now applied only while pulling (fixes the agent sheet AND every similarly-trapped modal). `BUG-7` **safe-area insets were inert** — the layout `viewport` lacked `viewport-fit=cover`, so every `env(safe-area-inset-*)` collapsed to 0 (header under the notch, tab labels under the home indicator); added `width/initial-scale/viewport-fit`. Ran a full mobile-readiness audit (subagent) and fixed h-overflow from long unbroken tokens (email/upload bodies + metadata rows), raised the source-doc Sheet above the feedback FAB with bottom safe-area padding, and truncated Sync run labels. `FEAT-33` **Ask consolidated to ONE box**: the single Ask box lives in the header on every screen; the Ask landing shows only recent questions (no seed grid, no info banner); the reset/"New" control lives inside the box. All typecheck+build-clean, committed, and pushed to `main` → live on **bellwood-mayor.vercel.app**. Earlier tonight — **the Mayor's live app brought to full parity for Tuesday.** Merged `agent-focus-and-sync-fixes` → `main` (clean fast-forward; build verified) so **bellwood-mayor.vercel.app now runs everything** we built this sprint. Applied migrations **014–017** to the live `bellwood-mayor` Supabase (81k real messages, untouched) — all additive, RLS deny-all verified via `get_advisors` (BUG-3 guard: none anon-readable); `015` also added the `connector_accounts.config` column the sync-retry needs. Confirmed the new build is serving by route-diff (`/api/triage` absent in old `main`, now 401 on his site). `FEAT-32` **Outlook login now mirrors Google** (`prompt=consent` → reliable refresh token + inline "Consent on behalf of your organization" for a tenant-admin mayor) and a graceful `/auth/error` screen replaces the raw `AADSTS65001`. Earlier today: `FEAT-30/31` triage on the Hub + every report link resolves, `BUG-4` (triage opened nothing — source_ref vs message_id), `BUG-5` (citation laundering via agent_memory), sync self-heals transient failures with real error cause. **OPEN before Tuesday:** (1) triage tables on the mayor DB are created-but-empty — the registered cron (pre-dawn + hourly) fills them automatically, or trigger now with the mayor's `CRON_SECRET`; (2) verify `RETRIEVAL_BACKEND=canonical` on bellwood-mayor (TASK-13, affects Ask only); (3) the one-time Outlook **admin consent** for the mayor's own mailbox. Prior: **Day session 2026-07-19: Triage reaches the Hub; every report email is a working link.** On branch `agent-focus-and-sync-fixes` (NOT merged/pushed): `FEAT-30` the ranked **"Needs you" triage preview** now tops the Hub (both surfaces), each item linking to the real email; `FEAT-31` the agent report's **"Agent responded"** sent list is now clickable (via `app.drafts.to_message_id`). Two link bugs fixed and verified live on RD's real mailbox: `BUG-4` triage links opened nothing (read returned canonical `message_id`; viewer resolves by `source_ref` — now exposes `source_ref`), and `BUG-5` **citation laundering** (a body-lifted message-id laundered through `canonical.agent_memory` into the trusted known-set survived the prune and rendered a dead link — the known-set is now validated against `canonical.messages`; re-ran all 9 agents → 36/36 citations resolve). Prior: **Night session 2026-07-18: agents became instructable.** `FEAT-27` **Focus + the one box** shipped on branch `agent-focus-and-sync-fixes` (8 commits, pushed, **NOT merged**): an agent is now configured by ONE plain-English instruction; a Haiku call derives the retrieval query from it at save time; retrieval runs semantically over the WHOLE archive instead of the since-cursor time window. `FEAT-29` **Google Security agent** — the first agent routed entirely by instruction (no StreamKey) — first live run on RD's real mailbox: **slice 0, digest 4**, and it surfaced a genuine account-recovery attempt that fired twice that day plus an unresolved breach notice. `DEC-15` **canonical retrieval cutover** (`RETRIEVAL_BACKEND=canonical`) — Ask had been querying the empty `poc` store on this stack; **`TASK-13` must verify the same env var on bellwood-mayor before trusting Ask there.** Two silent-failure classes fixed: `BUG-2` a duplicate message could stall a backfill **permanently** (cursor never advances), `BUG-3` `app.audit_log` was anon-readable (now encoded in `004`, matching the fix MH-2 applied by hand). `ISS-6` **fabricated metrics deleted** — the Ask right rail showed a hardcoded "92%" and invented gap cards ("Public Works CSV at 78% coverage") beside live answers; replaced with real recent searches. Also: `FEAT-28` sync progress (rate/min, ETA, mirrored-vs-searchable as two tracks), Wall attention ordering + per-agent "Reads" sources, shared agent voice (`lib/agents/voice.ts`), `docs/AGENT_TEST_PLAN.md`. Test rig: throwaway Supabase `bellwoodhub-agent-test` + RD's own Gmail, **5,400+ messages mirrored and embedded** (`DEP-4`: delete it, $10/mo). Prior close 2026-07-12 — **DEC-14: Zero-Body architecture decided** (the hub keeps the catalog, never the mail — bodies stop being stored in the cloud; live hydration from Graph/Gmail; FEAT-25 redesign Z1–Z4; on-prem sources get a Connector Gateway later, FEAT-26; full design `docs/ZERO_BODY_ARCHITECTURE.md`). Prior close 2026-07-06 — **Tuesday is a runbook, not a plan**: MH-D1…D5 decided · MH-1 code done (sentitems delta + `bf:` parity, 17 evals green) · **FEAT-24 Sync page shipped** (progress bars w/ real denominators, Voyage ETA, keep-going loop, Connect + Reconnect buttons) · **MH-2 DONE, verified live** (Supabase `BellwoodHub-Mayor` 18 migrations/RLS 29-29 · **bellwood-mayor.vercel.app** · native Vercel crons proven firing) · Entra app registered + env live on BOTH lanes (Microsoft sign-in available; permissions trimmed to least-privilege 7) · allowlist = RD interim + `aharvey@vil.bellwood.il.us` · **one codebase, three sites** (`f269bf7`). **OPEN before Tuesday:** village-IT admin consent · publish Google consent screen (then a fresh Gmail reconnect — pre-publish tokens keep the 7-day fuse) · the Mayor's Gmail (Monday) · first live Graph pull (rehearsal declined; first run is Tuesday) · close-out lockout (allowlist swap + `AUTH_SECRET` rotation) · TASK-12 password rotations wk of 7/13. Night close 2026-07-05 — the marathon session: **ING-4 live** (Voyage embeddings + Ask over real mail — proven on RD's own questions; the scheduler runs ingest+embed every 15 min autonomously via GitHub Actions + protection bypass), **agents became configuration** (FEAT-19 slice 2: prompt/urgency rules editable on the card · FEAT-21 skills upload · FEAT-20 plain-English cards), **DEC-13 vocabulary** (Agent / Capability / Connector — Staff Agents in three tinted collapsible sections + nav sub-menu), **Activity console** (the audit ledger live in-app), send-cage pill, dashboard email-agent box, formatted Ask answers, mobile above-the-fold pass, **Ask-as-mic** + desktop topbar Ask box. **Tuesday = Mayor Harvey onboarding** (MH board; decisions MH-D1…D5 pending; hand-out ready: `docs/MAYOR_ONBOARDING_OVERVIEW.md`). URLs: demo=**bellwood-hub.vercel.app** (public), pilot=**bellwood-hub-pilot.vercel.app** (SSO, auto-deploys on push to `live-pilot`)
...
| Web dev server | ✅ `http://localhost:3200` → `/chief` (200) |
| Live deploy | ✅ **bellwood-dev.vercel.app** — `/chief` 200 (`7d602de`, 2026-07-30) |
| Models | ✅ Opus 5 / Sonnet 5 / Haiku 4.5 via `TASK_PROFILE`; SDK 0.115.0 |
| MCP endpoint | ✅ `MCP_SECRET`-gated — 401 without key (was **open** to the world) |
| Calendar | ✅ 31 events mirrored; conflict flags live |
| Mail sync | ⛔ broken since 2026-07-23 (`gmail 404`), 462 behind (`ISS-11`) |
| Triage coverage | 🚧 400 of 125,459 messages (0.32%) — `ISS-9` |
| Sign-in (deployed) | ⛔ blocked on Google Console redirect URIs (`DEP-6`, owner RD) |
| Admin console | ✅ Models · Cost · Agent Rules · Skills · Sources (localStorage, demo-grade) |

**Top blocker (2026-07-30):** `DEP-6` — sign-in on the deployed app fails until RD adds the two redirect URIs in Google Console. Second: `ISS-11` Gmail sync has been down a week, so the corpus is stale.

*(historical)* **Top blocker:** none for the **Monday mayor demo** — it's built and running on the bulletproof JSON path. Open *product* items (connector, send, eval) are post-demo.
...
**2026-07-31 (overnight) — Ask becomes an agent · user-selectable model**

*Shipped*
- `FEAT-48` (done) — **Ask is now a tool loop, not a single-shot RAG pass.** `lib/ask-agent.ts` gives Claude the same tools the MCP server exposes (`search_email`, `read_email`, `person_or_place_history`) and lets it drive: search, read, search again. **Verified against Claude.ai as ground truth** — reproduces every figure of the Google-spend table (Jan $44.26 … Jul $42.43, **total $542.61**, including the April $36.95 + $100.00 split) across repeated runs, and goes further by comparing to 2025 and tracing the April/May spike to a Gemini API project. Encoded as a regression test in `eval/ask-agent.test.ts`.
- `FEAT-49` (done) — **The user picks the model, lower-right of the Ask box.** Default Opus 5. The catalog is read **live from the Models API**, not hardcoded — this repo already proved a static list rots (the Admin console still offered Sonnet 4.6 / Opus 4.8 and priced Opus at $15/$75, which was never Opus 5's rate). 11 models discovered live, including Fable 5, which no list in the repo knew about. The choice lives in `app.user_state`, **not** localStorage, because the cron agents have no browser — "the agents run with that model" only holds if a background job can read it.
- `FEAT-50` (done) — Models raised to **Opus 5 @ xhigh** for synthesis and the research loop; `research` profile added at 32k tokens.
- `BUG-13` (fixed) — **`/api/ask` was being killed in production.** A cross-reference ask measured **47s warm locally**; `maxDuration` was 60. Raised to 300.
- `BUG-14` (fixed) — **Mobile Ask failed silently.** `postJson` swallowed every error to `null` and `run()` never called `setErr`, so a timeout produced a spinner and then *nothing* — the reported "hang". Both shells now bound the request (290s) and say what happened.
- `BUG-15` (fixed) — **Three separate hardcoded `k = 8` caps** (route, `retrieval-canonical`, planner) silently capped completeness answers at 8 excerpts. Also fixed a regression I introduced where passing a bare date into the structured pass made it return "the 400 newest messages of any sender", swamping results with LinkedIn and Wayfair.
- `BUG-16` (fixed) — The `Searching` ladder ended at 7.4s, so a 47s question froze on one line for forty seconds. Extended, plus a **measured** elapsed clock (the stages are time-based; elapsed is the only real number, so it is the only one shown).
- `FEAT-51` (done) — `AnswerMd` gained fenced code blocks, inline code and links; the synthesizer may now use **tables** (the renderer always supported them — the prompt forbade them by omission) and asks **one** clarifying question rather than guessing at an unknown vendor. Verified live: it asked about "Versal" while correctly treating "SopaBase" as a typo.

*Findings*
- `NOTE-1` — **The gap was never the model.** The pipeline and Claude.ai run the *same* model; Claude.ai wins because it searches iteratively. Every heuristic added to close the gap (`COMPLETENESS_INTENT`, `senderPass`, `subjectPatterns`) was hand-coding one search strategy, and each broke on the next question shape. This is the same insight as `DEC-20` for the desks, and it now has a working proof.
- `ISS-16` (med, open) — **Attachment text is unreachable.** `canonical.messages` has no attachment column; Workspace invoices only link to a PDF. Claude.ai read those PDFs, we cannot. A *complete* Google total needs attachment extraction in ingestion. The agent correctly says so rather than guessing.
- `ISS-17` (low, open) — Agentic Ask costs **270s / 167k input tokens** on a research question, leaving only 30s of margin under the function limit. Worth streaming the answer so the wait is visible rather than a spinner.
- `ISS-18` (low, open) — `eval/ask-retrieval.test.ts` still has 2 failing checks on the *pipeline* path, which now matters much less since research questions bypass it. Decide: fix or retire.

*Decision*
- `DEC-21` (2026-07-31) — **The model is a user-facing setting**, deliberately reversing `DEC-19` for this one control. Rationale: it is a familiar affordance (Claude.ai puts it under the composer), it is the one knob with a real cost/quality tradeoff the user should own, and it is safe — every option is a valid model.


**2026-07-30 — personal-instance pivot, model upgrade, dashboard rebuild, security fix**

*Shipped*
- `FEAT-45` (done) — **Models → Opus 5 / Sonnet 5 with per-task capability profiles.** SDK `0.40.1 → 0.115.0`. A model-string swap alone would have 400'd every agent run: both models reject `temperature` (sent on every call) and default thinking ON, which shares the `max_tokens` budget and would have truncated the structured digests `validateRunOutput()` parses. Every per-model knob now lives in `TASK_PROFILE` (`lib/agents/constants.ts`), so a future generation is a table edit. Verified against the live API: Haiku 4.5 **rejects** `output_config.effort`; Opus 5 rejects `xhigh`/`max` effort with thinking disabled. `eval/profile-smoke.ts` exercises all 8 profiles.
- `FEAT-46` (done) — **Dashboard: one brief + three panels, shared by mobile and desktop.** Replaces the 7-widget grid. Mobile (`NeedsToKnowCard`) and desktop (`DashboardHub`) had drifted into different products; both now render the same components from `components/chief/dashboard/panels.tsx`. The brief leads (read first in the morning) with per-desk attribution, a **Run now** button on `/api/agents/run-now`, and the next scheduled pass (`nextAgentRun()`, 7 checks, UTC-based).
- `FEAT-47` (done) — **Calendar connected.** 31 events live; `/api/events` now returns `endDate`/`allDay`/`conflictsWith`. Overlap detection is pure (`lib/event-conflicts.ts`, 7 checks) and deliberately excludes all-day events, which would otherwise flag the whole week.
- `BUG-9` (**high**, fixed) — **Calendar ingest was a silent no-op for a week.** `cron/ingest-calendar` selected only `status='active'` accounts; the Gmail account sat at `status='error'` from an *unrelated* mail 404, so the every-2-hours cron ran and did nothing. Calendar rides the Gmail grant but is a separate capability and must fail independently. Second, compounding cause: the Google Calendar API was disabled on the Cloud project (enabled in console).
- `BUG-10` (**critical**, fixed) — **Production MCP endpoint was unauthenticated.** `bellwood-dev.vercel.app/api/mcp` served all 125,459 personal messages *and* `draft_reply` to any unauthenticated caller — confirmed by pulling a real invoice with no credentials. `MCP_SECRET` set and verified live: 401 without the key, 200 with it.
- `BUG-11` (fixed) — **OAuth `redirect_uri_mismatch`.** `AUTH_URL` was unset, so NextAuth derived the origin from whichever hostname was hit; per-deployment URLs can never be pre-registered. Pinned to `https://bellwood-dev.vercel.app`. *Console redirect URIs remain RD's action — see `DEP-6`.*

*Found, not yet fixed*
- `ISS-9` (**high**) — **Triage covers 400 of 125,459 messages (0.32%).** `MAX_THREADS = 400` + `WINDOW_DAYS = 45` in `lib/triage/run.ts`. The old Priority Matrix reported "97% FYI" — 97% *of the 400*, presented as the whole mailbox. The bulk-mail classifier already exists (`FYI_SENDER` regex) and is free; it is simply starved of scope.
- `ISS-10` (**high**) — **The privacy wall is not implemented in the live path.** `canonical.messages` has **no** mailbox/lane column and retrieval applies **no** lane filter; `mailbox_id` is read in only 3 places, all for a "walled" *label*. Moving an account to `biz` would change a badge and remove nothing from search. Material for the mayor build, where FOIA separation is a stated non-negotiable.
- `ISS-11` (**high**) — **Gmail sync broken since 2026-07-23** — `gmail 404: Requested entity was not found`, 462 messages behind. Root cause of `BUG-9`'s trigger condition.
- `ISS-12` (med) — **Municipal desks are dead weight on the personal instance.** police/fire/council/constituent run 111× each and return the honest-empty sentinel every time; only 3 of 9 desks invoke a model.
- `ISS-13` (med) — **Branding mismatch.** The live app greets "Good evening, Mayor Harvey" over a Bellwood logo on an instance now defined as RD's personal assistant.
- `ISS-14` (low) — `/api/dashboard` is dead code; no component fetches it.

*Decisions*
- `DEC-18` (2026-07-30) — **The live instance is RD's personal Chief of Staff.** Bellwood is the demo persona only; the mayor's version becomes a separate copy/release, migrated later. Settled after finding the corpus holds exactly **1** bellwood-addressed message and **2** `.gov` messages out of 125,459 — the "retrieval quality" problem was a *missing municipal corpus*, not a ranking failure.
- `DEC-19` (2026-07-30) — **Configuration stays with the developer.** Agents, models, and connectors are configured at the prompt/deploy level in Claude Code, not exposed as end-user settings — exposing them creates noise and confusion. Directly motivates retiring most of `AdminPanel` (550 lines) and `AgentsPage` (879 lines).
- `DEC-20` (2026-07-30) — **Do not move the desks to Managed Agents.** They are single structured completions, not tool loops; there is no loop to hand over, and migrating would move citation validation, the audit ledger, and the send gate off our infrastructure. Instead add a tool-use loop over the five functions the MCP server already wraps.

*Risks & dependencies*
- `BUG-12` (**high**, open) — **The MCP shared-secret guard fails OPEN.** `app/api/[transport]/route.ts:181` is `if (!secret) return true;` — an environment without `MCP_SECRET` serves the whole corpus and `draft_reply` to anyone. `MCP_SECRET` was set on **bellwood-dev** (verified 401), but **bellwood-hub** and **bellwood-mayor** were both answering 200 unauthenticated and were not fixed, and local dev has no secret either. Fix is one character of intent: fail closed, then set the secret per environment. Found by the `RSK-7` adversarial audit.
- `ISS-15` (med, open) — **Several GET handlers write.** `GET /api/triage/senders` seeds `app.important_senders` on first read (product state, not a cache); `/api/wall`, `/api/queue`, `/api/inbox`, `/api/email`, `/api/agents/run` each write an `app.audit_log` row per GET, and the middleware itself POSTs an `access.page` row per page view. A "block non-GET" read-only gate therefore does **not** produce a read-only user. Also: Next 14 auto-implements HEAD by invoking the GET handler verbatim, so HEAD is not inert.

- `RSK-7` (**high** · likelihood med × impact high) — **A read-only role that leaks a mutation lets a viewer send mail as RD or run up model spend.** The gate must be enforced server-side at the middleware chokepoint and cover Server Actions, side-effecting GETs, and every path in the middleware `OPEN` allowlist. Adversarial audit commissioned. *Audit complete (8 agents, 112 surfaces, 81 mutating, 5 realistic bypasses).* The naive gate would have been a **permanent no-op**: NextAuth v5 with `strategy:"jwt"` does not copy JWT claims onto the Session without a `session()` callback, and none exists — so `session.user.role === "viewer"` is always `undefined === "viewer"` → false → full write access. Worse, role must be derived from **identity** (an env allowlist resolved in the `jwt`/`session` callback, defaulting to viewer) rather than from the login method: `/api/auth/` is necessarily in the middleware `OPEN` list, so a viewer could simply re-authenticate with Google and return with no role claim at all. That sign-in is itself a mutation — the `jwt` callback writes `connector_accounts` and can flip an errored account back to `active`. *Mitigation: read the raw token, not the Session; invert to fail closed (anything not explicitly a writer is a viewer); cover Server Actions, the GET-writes in `ISS-15`, and `BUG-12`.*
- `DEP-6` (**blocking**, owner **RD**) — **Google Console redirect URIs.** Sign-in on the deployed app fails until `https://bellwood-dev.vercel.app/api/auth/callback/google` and `http://localhost:3200/api/auth/callback/google` are added to OAuth client `32902323271-bhekte5q27…`.
...
- **2026-07-31 (overnight — the agent that searches back)** — Closed the loop on the evening's question "do we need a better model?" with a definitive **no**. Proof: given the same Gmail account and the same model, Claude.ai answered "break down my Google spend by month" correctly and this app did not — because Claude.ai *searched four times*, adapting each query, and the app searched once. Rebuilt Ask as a tool loop (`FEAT-48`); it now reproduces Claude.ai's answer **figure for figure** ($542.61 total, April's two-charge split intact) and is locked in as a regression test. Also shipped the user-facing model picker with a **live** catalog from the Models API (`FEAT-49`, `DEC-21`) — no more hardcoded lists going stale — persisted server-side so the cron desks honour it. Fixed the production Ask timeout (`BUG-13`), the silent mobile failure that read as a hang (`BUG-14`), three hidden `k=8` caps plus a regression of my own (`BUG-15`), and the frozen progress ladder (`BUG-16`). Deployed `5ef10b5` → bellwood-dev, `/chief` 200, MCP still 401. Outstanding: `DEP-6` Google Console redirect URIs (owner RD) still blocks sign-in, and `ISS-16` attachment extraction blocks a *complete* spend total.
- **2026-07-30 (personal-instance pivot · Opus 5 · dashboard rebuild · an open door closed)** — The session started as "simplify the dashboard" and turned into a re-framing of what the product *is*. Root-cause work on "retrieval quality is poor" found the live corpus holds **1** bellwood-addressed message and **2** `.gov` out of 125,459 — semantic search had been returning the honest best match over a corpus with no municipal mail in it. `DEC-18`: the live instance is RD's **personal** Chief of Staff; the mayor's build becomes a separate release. Reassuringly, the desks proved honest under that discovery — police/fire returned empty digests with "Checked — nothing new", no fabricated citations, exactly the R4 honest-gap behaviour, verified against real data. Shipped `FEAT-45` (Opus 5 / Sonnet 5 + per-task capability profiles, SDK 0.115.0 — a naive model swap would have 400'd every run on `temperature`), `FEAT-46` (one brief + three panels, mobile and desktop finally sharing components), `FEAT-47` (calendar live, 31 events, conflict detection). Fixed `BUG-9` (calendar ingest silently disqualified by an unrelated *mail* error — a week of no-ops), `BUG-10` (**critical** — production MCP served the entire personal mailbox and `draft_reply` unauthenticated; confirmed by pulling a real invoice with no credentials, now 401), `BUG-11` (`AUTH_URL` unset → `redirect_uri` varied by hostname). Logged `ISS-9`…`ISS-14` and `RSK-7`. Committed in 4 chunks and deployed: `9845753..7d602de` → **bellwood-dev.vercel.app**, `/chief` 200, MCP 401-without-key verified. Still owed by RD: `DEP-6` Google Console redirect URIs.
- **2026-07-21 (night — bellwood-dev in the cloud; the Brief becomes the product)** — `DEP-5` created Vercel project **bellwood-dev** (RD's dev deployment, own test DB, auth ON, 12 env vars, always-on crons) and deployed the evening's run there. Shipped: `FEAT-36` voice notes end-to-end (smart Ask routing + pencil button + confirm-first + Haiku cleanup + briefing integration, migrations 018); `FEAT-38` the Brief front page (Chomsky nameplate, clock edition, dateline clock, drop cap, article typography, Agent Analysis & Updates band); `FEAT-39` desk stories → Brief agent ranking by mayoral importance, bylines, story-click → agent report with source-email chip; `FEAT-40` Swipe the Brief (horizontal snap story mode); `FEAT-41` Show the work — run-recorded model/timing/tokens/prompt/response/validation (019) + the brain-button trace panel, verified live. UX: Agents peek-fold + main-menu entry, Needs You → Email Actions, header date/time, stacking fixes. Mayor's `main` NOT updated tonight — his DB needs 018+019 before the next push.
- **2026-07-21 (the Hub becomes intelligence — Needs to Know + news-brief reports, deployed)** — `FEAT-34` Needs to Know: live CoS briefing leads the Hub (narrative in the tunable persona, ranked email issues with working links, actions, events), combining the old two sections; live assembly in `lib/morning-live.ts`, fixture fallback so the Hub never blanks; tested T1–T5 against the real mailbox. `FEAT-35` agent digests restyled as news briefs (title + facts + NEW/UPDATE from agent memory), schema backward-compatible. `BUG-8` `/auth/error` un-gated (its audience has no session) — found by post-deploy probe, fixed, verified 200. Ask hero buttons (desktop raised gold orb, mobile centered FAB) shipped same push. Deployed `main` → bellwood-mayor (`7059869`). Started `FEAT-36` voice notes for walk-ins (migration written). Calendar mirror diagnosed: Google Calendar API disabled in the OAuth project (403), not a scope problem — one-click enable pending.
- **2026-07-20 (late night — mobile-readiness pass + Ask consolidation)** — Fixed the reported mobile bug (`BUG-6`: agent card off-screen — `PullToRefresh` at-rest transform trapping all fixed modals) and ran a full mobile audit. `BUG-7`: `viewport-fit=cover` added so `env(safe-area-inset-*)` actually works (header/nav/sheets were collapsing to 0 on notched phones). Fixed h-overflow from long unbroken tokens in email/upload bodies + metadata rows; raised the mobile source-doc Sheet above the feedback FAB with bottom safe-area padding; truncated Sync run labels. `FEAT-33`: Ask consolidated to one header box — landing shows only recent questions, reset/"New" lives inside the box. Also fixed OAuth/allowlist confusion: the live `bellwood-mayor` Supabase is populated with **RD's Gmail** (81k msgs, gmail connector `rdawson@…`), not the mayor's Outlook (never connected — pending admin consent). Committed + pushed to `main` → deployed to bellwood-mayor.vercel.app. Build verified.
- **2026-07-20 (night — mayor app to full parity + Outlook login fix)** — Merged the sprint branch to `main` (clean FF) and deployed to the mayor's live app. Applied migrations 014–017 to the live `bellwood-mayor` Supabase (additive, RLS deny-all verified). `FEAT-32`: Outlook login mirrors Google (`prompt=consent`) + inline admin org-consent + graceful `/auth/error`. Diagnosed the "admin approval for Outlook" blocker as the tenant's user-consent policy (one-time IT/admin consent, unavoidable; app already least-privilege).
